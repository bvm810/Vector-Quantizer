...
void trainingCleanup(Img **imgs, BlockMatrix **vImgs, Cluster *clusters, unsigned K, unsigned nImg) {
    unsigned i;
...
void train(unsigned K, unsigned blockWidth, unsigned blockHeight, char *imgPath, int seed, char *cbookName, char *mode) {
    ...
    trainingCleanup(imgs, vImgs, clusters, K, imgCount); // todo problemas aqui ...
}

int main (int argc, char **argv) {
    unsigned i, j, k, blockWidth, blockHeight;
